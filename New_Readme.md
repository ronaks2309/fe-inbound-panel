...
Notes: Startup seeds Client(id="demo-client"). DB lives at backend/vapi_dashboard.db. Delete the file to reset local data.
...
- CallDashboard.tsx
  ...
  - Opens WebSocket to ${backendUrl}/ws/dashboard; handles call-upsert and transcript-update messages to keep local state in sync.
  - Actions per call:
    - Active (in-progress/ringing/queued/forwarding): Listen (opens ListenModal if listen_url), Transcript (opens TranscriptModal if live/final text), Take Over (force transfer; currently posts a fixed agent number +16504848853; change as needed).
    - Ended: Call Recording button placeholder (enable once recording_url is surfaced), View Transcript.
  - TranscriptModal picks final_transcript when ended; otherwise shows live_transcript if present.
  - ListenModal opens call.listen_url WebSocket, expects 16-bit PCM mono at 32000 Hz, applies a low-pass filter + gain, schedules audio chunks to avoid gaps, and stops WS + AudioContext on close. Accepts text frames for metadata; binary frames are decoded to Float32 audio buffers.
- Styling: Tailwind v4 via @import "tailwindcss";. Legacy Vite CSS assets remain but dashboard styling is Tailwind-based.

## Expected VAPI Webhook Payloads (examples)
- Status update
  `json
  {
    "message": {
...
- Parameterize force-transfer agent number in UI/DB instead of the current hard-coded value.
...
- Replace SQLite with Postgres for production; add migrations.
